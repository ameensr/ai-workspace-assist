...
export async function chatCompatibleService({ provider, endpoint, apiKey, prompt, options = {} }) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const error = new Error(normalizeProviderError(provider, response.status, body));
